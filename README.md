...
### 3. Execution Mode
Agents don't just talk; they act. If a discussion requires validation (writing a script, creating a mockup, or drafting a financial model), an agent must request execution:
`[ACTION REQUIRED: Execute? Let me generate a financial model...]`
The action is triggered only upon explicit approval from the CEO.
